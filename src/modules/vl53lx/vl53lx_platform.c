...
LOG_MODULE_DECLARE(VL53LX, CONFIG_SENSOR_LOG_LEVEL);

VL53LX_Error VL53LX_WriteMulti(VL53LX_DEV Dev, uint16_t index, uint8_t *pdata,
				 uint32_t count)
{
...
VL53LX_Error VL53LX_ReadMulti(VL53LX_DEV Dev, uint16_t index, uint8_t *pdata,
				uint32_t count)
{
	VL53LX_Error Status = VL53LX_ERROR_NONE;
	int32_t status_int;
	...
	// status_int = i2c_burst_read(Dev->i2c, Dev->I2cDevAddr, index, pdata, count) index is uint16_t
	status_int = i2c_write_read(Dev->i2c, Dev->I2cDevAddr,
			      &I2CBuffer, sizeof(I2CBuffer),
			      pdata, count);
...
VL53LX_Error VL53LX_WrByte(VL53LX_DEV Dev, uint16_t index, uint8_t data)
{
	...
	//status_int = i2c_reg_write_byte(Dev->i2c, Dev->I2cDevAddr, index, data) BUG index is uint16_t
	status_int = i2c_write_read(Dev->i2c, Dev->I2cDevAddr,
			      &I2CBuffer, sizeof(I2CBuffer),
			      &data, sizeof(&data));
	if (status_int < 0) {
				 ...
				 uint8_t AndData, uint8_t OrData)
{
	...
	status_int = VL53LX_RdByte(Dev, index, &data);
	if (status_int < 0) {
		Status = VL53LX_ERROR_CONTROL_INTERFACE;
		LOG_ERR("VL53LX_RdByte failed (%d)", Status);
	}

	if (Status == VL53LX_ERROR_NONE) {
		data = (data & AndData) | OrData;
		status_int = VL53LX_WrByte(Dev, index, data);
		if (status_int != 0) {
			Status = VL53LX_ERROR_CONTROL_INTERFACE;
			LOG_DBG("VL53LX_WrByte failed.(%d)", Status);
		}
	}
...
VL53LX_Error VL53LX_RdByte(VL53LX_DEV Dev, uint16_t index, uint8_t *data)
{
	...
	//status_int = i2c_reg_read_byte(Dev->i2c, Dev->I2cDevAddr, index, data) // index is uint16_t
	status_int = i2c_write_read(Dev->i2c, Dev->I2cDevAddr,
			      		&I2CBuffer, sizeof(I2CBuffer),
			     		data, sizeof(*data));
	if (status_int < 0) {
...
VL53LX_Error VL53LX_RdWord(VL53LX_DEV Dev, uint16_t index, uint16_t *data)
{
	...
	//status_int = i2c_burst_read(Dev->i2c, Dev->I2cDevAddr, index, buf, 2) index is uint16_t
	status_int = i2c_write_read(Dev->i2c, Dev->I2cDevAddr,
			      &I2CBuffer, sizeof(I2CBuffer),
			      &buf, 2);
	if (status_int < 0) {
    Status = VL53LX_ERROR_CONTROL_INTERFACE;
    LOG_ERR("i2c_burst_read failed (%d)", Status);
    return -EIO;
	}
...
VL53LX_Error  VL53LX_RdDWord(VL53LX_DEV Dev, uint16_t index, uint32_t *data)
{
	...
	//status_int = i2c_burst_read(Dev->i2c, Dev->I2cDevAddr, index, buf, 4) index is uint16_t
	status_int = i2c_write_read(Dev->i2c, Dev->I2cDevAddr,
			      &I2CBuffer, sizeof(I2CBuffer),
			      &buf, 4);
	if (status_int < 0) {
...
#if 0 // Kann später gelöscht werden
